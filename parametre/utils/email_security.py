...
class EmailValidator:
    ...
    def is_valid_email(cls, email: str) -> bool:
        ...
        if domain in cls.SUSPICIOUS_DOMAINS:
            logger.warning("⚠️ Domaine suspect détecté : %s", domain)
            return False
    ...
    def sanitize_email_list(cls, emails: list) -> list:
        ...
        for email in emails:
            if cls.is_valid_email(email):
                ...
                logger.warning("⚠️ Email invalide filtré : %s", email)
...
class EmailContentSanitizer:
    ...
    def sanitize_url(url: str) -> str:
        ...
        if not url.startswith(('http://', 'https://', '/')):
            logger.warning("⚠️ URL suspecte : %s", url)
            return ''
        ...
        for protocol in dangerous_protocols:
            if protocol in url_lower:
                logger.warning("⚠️ URL avec protocole dangereux détecté : %s", url)
                return ''
...
class EmailRateLimiter:
    ...
    def check_user_limit(cls, user_id: int) -> bool:
        ...
        if count >= cls.MAX_EMAILS_PER_HOUR_USER:
            logger.warning("⚠️ Limite d'emails dépassée pour l'utilisateur %s", user_id)
            return False
    ...
    def check_test_email_limit(cls, user_id: int) -> bool:
        ...
        if count >= cls.MAX_TEST_EMAILS_PER_MINUTE:
            logger.warning("⚠️ Limite de tests dépassée pour l'utilisateur %s", user_id)
            return False
